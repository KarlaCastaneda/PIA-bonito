...
LRESULT CALLBACK FiltroCosumos(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam);
...
LRESULT CALLBACK WindowProc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
    ...
    case WM_COMMAND:
        ...
        {
		case 1012: { //VALIDACION PARA EL INICIO DE SESION
			char usuarios_nombre[15];
			char usuarios_contrasenia[10];
			char usuarios_comercio[15];
				...
				if (Usuario_aux == nullptr) {
					...
					if (strcmp(Usuario_aux->usuarios_contrasenia, usuarios_contrasenia) == 0)
					{
						if (strcmp(Usuario_aux->usuarios_comercio, usuarios_comercio) == 0) {
							...
							if (strcmp(Usuario_aux->usuarios_primeregistro, "0") == 0) {//SE VALIDA SI YA HABIA INICIADO SESION ANTES
								EndDialog(hwnd, 0);
        ...
        case 1005:{ //DE INICIAR SESION A REGISTRARSE
            EndDialog(hwnd, 0);
		...
		case 1013: { //DE INICIAR SESION A SALIR
		...
		case 1006: { //Boton de Aceptar registro
			...
			if (lengthNombre <= 0 || lengthNombre > 15)//valida nombre
			{
				...
				else
				{
					...
					if (Usuario_aux != nullptr) {
						MessageBox(hwnd, "Usuario ya existente, intente con otro nombre", "AVISO", MB_OK | MB_ICONERROR);
					}
					else {
		...
		case WM_INITDIALOG: {//rellena lista de usuarios
			...
			while (Cliente_aux != nullptr) {
				if (strcmp(Usuario_comercio_c, Cliente_aux->Cliente_comercio) == 0) {
					SendMessage(GetDlgItem(hwnd, LISTA_SumaComprasMENU), LB_ADDSTRING, 0, (LPARAM)Cliente_aux->Cliente_nombre);
					CClientes++;
				}
			...
			SetDlgItemText(hwnd, EDIT_NuClientesMENU, ClientesTotal);

			Promocion_aux = Promocion_inicio;
			while (Promocion_aux != nullptr) {
				if (strcmp(Usuario_comercio_c, Promocion_aux->Promocion_comercio) == 0) {
					if (strcmp(Promocion_aux->Promocion_estatus, "Activa") == 0)
					{
						CPA++;
					}
					if (strcmp(Promocion_aux->Promocion_estatus, "Inactiva") == 0)
					{
						CPI++;
					}
				}
			...
			SetDlgItemText(hwnd, EDIT_PromoHabMENU, Act);

			sprintf_s(PromoIn, "%d", CPI);
			strcat_s(In, PromoIn);
			SetDlgItemText(hwnd, EDIT_PromoDeshabMENU, In);
			...
			if (strcmp(NUsuario->usuarios_tipos, "Cajero") == 0)
			{
				...
				EnableMenuItem(GetMenu(hwnd), 40010, MF_DISABLED | MF_BYCOMMAND);
			...
			while (Consumo_aux != nullptr) {
				if (strcmp(Usuario_comercio_c, Consumo_aux->Consumo_comercio) == 0) {
					Total = atof(Consumo_aux->Consumo_total);
					CST = CST + Total;
				}
			...
			SetDlgItemText(hwnd, EDIT_TotalConsumosMENU, TotalC);

			HBITMAP bmp;
			bmp = (HBITMAP)LoadImage(NULL, NUsuario->usuarios_foto, IMAGE_BITMAP, 70, 70, LR_LOADFROMFILE);
			SendDlgItemMessage(hwnd, FOTOMENU, STM_SETIMAGE, IMAGE_BITMAP, (LPARAM)bmp);
			SetDlgItemText(hwnd, STATIC_ManejadorMenu, NUsuario->usuarios_nombre);

		}break;

		case WM_COMMAND: {
			char textoList[50] = { 0 };
			if (LOWORD(wParam) == LISTA_SumaComprasMENU) {
				...
				case LBN_DBLCLK: {
					...
					indice = SendDlgItemMessage(hwnd, LISTA_SumaComprasMENU, LB_GETCURSEL, 0, 0);
					SendDlgItemMessage(hwnd, LISTA_SumaComprasMENU, LB_GETTEXT, indice, (LPARAM)textoList);
					...
					while (Consumo_aux != nullptr) {
						if (strcmp(Usuario_comercio_c, Consumo_aux->Consumo_comercio) == 0) {
							if (strcmp(textoList, Consumo_aux->Consumo_cliente) == 0) {
								...
								MM = atof(Consumo_aux->Consumo_total);
								TT = TT + MM;
					...
					SetDlgItemText(hwnd, 1164, SumaCliente);
			...
			case 40009: {
				EndDialog(hwnd, 0);
				DialogBox(hInstGlobal, MAKEINTRESOURCE(IDD_DIALOG15), hwnd, filtro);
			}
					  break;
			case 40010: {
				EndDialog(hwnd, 0);
	...
	case WM_COMMAND:
		...
		case 1018: {
			...
			EnableWindow(GetDlgItem(hwnd, 1005), true);
			  ...
			  break;
		case 1021:
		{//REGISTRAR ACEPTANDO LOS CAMBIOS
			int CLetras = 0;
			int CNumeros = 0;
			int CCaracteres = 0;
			int CEspacios = 0;
...
LRESULT CALLBACK AltaConsumos(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam) {
	...
	case WM_COMMAND: {
		...
		if (LOWORD(wParam) == 1084) {
			...
			GetDlgItemText(hwnd, EDIT_ClienteAltaConsumo, Nuevo_consumo->Consumo_cliente, sizeof(Nuevo_consumo->Consumo_cliente));
			...
			GetDlgItemText(hwnd, EDIT_IDAltaConsumo, Nuevo_consumo->Consumo_ID, sizeof(Nuevo_consumo->Consumo_ID));
...
LRESULT CALLBACK FiltroCosumos(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam) 
{
	...
	case WM_INITDIALOG: {//rellena lista de usuarios
		...
		while (Cliente_aux != nullptr) {
			if (strcmp(Usuario_comercio_c, Cliente_aux->Cliente_comercio) == 0) {
				SendMessage(GetDlgItem(hwnd, IDC_LIST1), LB_ADDSTRING, 0, (LPARAM)Cliente_aux->Cliente_nombre);
		...
		HBITMAP bmp;
		bmp = (HBITMAP)LoadImage(NULL, NUsuario->usuarios_foto, IMAGE_BITMAP, 70, 70, LR_LOADFROMFILE);
		SendDlgItemMessage(hwnd, FOTO10, STM_SETIMAGE, IMAGE_BITMAP, (LPARAM)bmp);
		SetDlgItemText(hwnd, BOTON_ModificarListaClientes, NUsuario->usuarios_nombre);
	}
	...
	case WM_COMMAND: {
		if (LOWORD(wParam) == IDC_LIST1) {
			...
			case LBN_DBLCLK: {
				...
				SetDlgItemText(hwnd, EDIT_FiltroConsumosCliente, "");
				SetDlgItemText(hwnd, EDIT_FiltroConsumosMonto, "");
				SetDlgItemText(hwnd, EDIT_FiltrosConsumoSubtotal, "");
				SetDlgItemText(hwnd, EDIT_FiltroConsumoDesc, "");
				SetDlgItemText(hwnd, EDIT_FiltroConsumoTotal, "");
				SetDlgItemText(hwnd, EDIT_FiltroConsumoFecha, "");
				int indice = 0;
				...
				while (Consumo_aux != nullptr) {
					if (strcmp(textoList, Consumo_aux->Consumo_cliente) == 0)
					{
						SendMessage(GetDlgItem(hwnd, IDC_LIST2), LB_ADDSTRING, 0, (LPARAM)Consumo_aux->Consumo_ID);
					}
					Consumo_aux = Consumo_aux->Consumo_sig;
				}
		...
		if (LOWORD(wParam) == IDC_LIST2) {
			...
			case LBN_DBLCLK: {
				...
				SendDlgItemMessage(hwnd, IDC_LIST2, LB_GETTEXT, indice, (LPARAM)textoList);
				...
				{
					SetDlgItemText(hwnd, EDIT_FiltroConsumosCliente, Consumo_aux->Consumo_cliente);
					SetDlgItemText(hwnd, EDIT_FiltroConsumosMonto, Consumo_aux->Consumo_monto);
					SetDlgItemText(hwnd, EDIT_FiltrosConsumoSubtotal, Consumo_aux->Consumo_subtotal);
					SetDlgItemText(hwnd, EDIT_FiltroConsumoDesc, Consumo_aux->Consumo_descuento);
					SetDlgItemText(hwnd, EDIT_FiltroConsumoFecha, Consumo_aux->Consumo_fecha);
					SetDlgItemText(hwnd, EDIT_FiltroConsumoTotal, Consumo_aux->Consumo_total); 
				}
		...
		if (LOWORD(wParam) == BTN_FiltroConsumoSalir) {
			EndDialog(hwnd, 0);
			DialogBox(hInstGlobal, MAKEINTRESOURCE(IDD_DIALOG3), hwnd, VentanaInicio);
		}
...
LRESULT CALLBACK filtro(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_INITDIALOG: {//rellena lista de usuarios
		...
		while (Cliente_aux != nullptr) {
			if (strcmp(Usuario_comercio_c, Cliente_aux->Cliente_comercio) == 0) {
				SendMessage(GetDlgItem(hwnd, 101), LB_ADDSTRING, 0, (LPARAM)Cliente_aux->Cliente_nombre);
			}
			Cliente_aux = Cliente_aux->Cliente_sig;
		}
		...
		HBITMAP bmp;
		bmp = (HBITMAP)LoadImage(NULL, NUsuario->usuarios_foto, IMAGE_BITMAP, 70, 70, LR_LOADFROMFILE);
		SendDlgItemMessage(hwnd, 110, STM_SETIMAGE, IMAGE_BITMAP, (LPARAM)bmp);
		SetDlgItemText(hwnd, 111, NUsuario->usuarios_nombre);
	}break;
	case WM_COMMAND: {
		if (LOWORD(wParam) == 101) {
			...
			case LBN_DBLCLK: {
				...
				SendMessage(GetDlgItem(hwnd, 102), LB_RESETCONTENT, 0, 0);
				SetDlgItemText(hwnd, 103, "");
				SetDlgItemText(hwnd, 104, "");
				SetDlgItemText(hwnd, 105, "");
				SetDlgItemText(hwnd, 106, "");
				...
				while (Consumo_aux != nullptr) {
					if (strcmp(textoList, Consumo_aux->Consumo_cliente) == 0)
					{
						SendMessage(GetDlgItem(hwnd, 102), LB_ADDSTRING, 0, (LPARAM)Consumo_aux->Consumo_ID);
					}
					Consumo_aux = Consumo_aux->Consumo_sig;
				}
		...
		if (LOWORD(wParam) == 102) {
			...
			case LBN_DBLCLK: {
				...
				indice = SendDlgItemMessage(hwnd, 102, LB_GETCURSEL, 0, 0);
				SendDlgItemMessage(hwnd, 102, LB_GETTEXT, indice, (LPARAM)textoList);
				...
				{
					SetDlgItemText(hwnd, 103, Consumo_aux->Consumo_cliente);
					SetDlgItemText(hwnd, 104, Consumo_aux->Consumo_monto);
					SetDlgItemText(hwnd, 105, Consumo_aux->Consumo_subtotal);
					SetDlgItemText(hwnd, 106, Consumo_aux->Consumo_descuento);
					SetDlgItemText(hwnd, 107, Consumo_aux->Consumo_fecha);
					SetDlgItemText(hwnd, 108, Consumo_aux->Consumo_total);
				}
...
LRESULT CALLBACK ListaPromocion(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam) {
	...
	if (strcmp(NUsuario->usuarios_tipos, "Cajero") == 0)
	{
		EnableWindow(GetDlgItem(hwnd, 1150), false);
		EnableWindow(GetDlgItem(hwnd, 1148), false);
	...
	while (Promocion_aux != nullptr) {
		if (strcmp(Usuario_comercio_c, Promocion_aux->Promocion_comercio) == 0) {
			SendMessage(GetDlgItem(hwnd, 1147), LB_ADDSTRING, 0, (LPARAM)Promocion_aux->Promocion_nombre);
		}
		Promocion_aux = Promocion_aux->Promocion_sig;
	}
	...
	HBITMAP bmp;
	bmp = (HBITMAP)LoadImage(NULL, NUsuario->usuarios_foto, IMAGE_BITMAP, 70, 70, LR_LOADFROMFILE);
	SendDlgItemMessage(hwnd, 1145, STM_SETIMAGE, IMAGE_BITMAP, (LPARAM)bmp);
	SetDlgItemText(hwnd, 1146, NUsuario->usuarios_nombre);

	}break;
	case WM_COMMAND: {
		if (LOWORD(wParam) == 1147) {
			switch (HIWORD(wParam)) {
			case LBN_DBLCLK: {
				SendDlgItemMessage(hwnd, 1144, BM_SETCHECK, (WPARAM)BST_UNCHECKED, 0);
				SendDlgItemMessage(hwnd, 1151, BM_SETCHECK, (WPARAM)BST_UNCHECKED, 0);

				int indice = 0;

				indice = SendDlgItemMessage(hwnd, 1147, LB_GETCURSEL, 0, 0);
				SendDlgItemMessage(hwnd, 1147, LB_GETTEXT, indice, (LPARAM)textoList);

				Promocion_aux = Promocion_inicio;
				while (Promocion_aux != nullptr && strcmp(textoList, Promocion_aux->Promocion_nombre) != 0) {
					Promocion_aux = Promocion_aux->Promocion_sig;
				}
				...
				{
					SetDlgItemText(hwnd, 1139, Promocion_aux->Promocion_nombre);
					SetDlgItemText(hwnd, 1142, Promocion_aux->Promocion_fecha);
					SetDlgItemText(hwnd, 1140, Promocion_aux->Promocion_monto);
					SetDlgItemText(hwnd, 1141, Promocion_aux->Promocion_descuento);

					if (strcmp(Promocion_aux->Promocion_estatus, "Activa") == 0)
					{
						SendDlgItemMessage(hwnd, 1144, BM_SETCHECK, (WPARAM)BST_CHECKED, 0);
					}
					if (strcmp(Promocion_aux->Promocion_estatus, "Inactiva") == 0)
					{
						SendDlgItemMessage(hwnd, 1151, BM_SETCHECK, (WPARAM)BST_CHECKED, 0);
					}
				}
		...
		if (LOWORD(wParam) == 1138) {//Registrar
			int indice = 0;
			...
			while (Promocion_aux != nullptr && strcmp(Promocion_aux->Promocion_nombre, textoList) != 0) {
			...
			else
			{
				...
				if (Opcion == IDYES) {
					if (IsDlgButtonChecked(hwnd, 1144) == BST_CHECKED)
					{
						strcpy_s(Promocion_aux->Promocion_estatus, "Activa");
					}
					if (IsDlgButtonChecked(hwnd, 1151) == BST_CHECKED)
					{
						strcpy_s(Promocion_aux->Promocion_estatus, "Inactiva");
					}
		...
		if (LOWORD(wParam) == 1148) {//Eliminar
			int indice = 0;
			...
			if (Opcion == IDYES) {
				...
				SendMessage(GetDlgItem(hwnd, 1147), LB_DELETESTRING, (WPARAM)indice, 0);
				SendMessage(GetDlgItem(hPrinGlob, 1147), LB_RESETCONTENT, 0, 0);
				Escribir_Binario_Promocion();

				Promocion_aux = Promocion_inicio;
				while (Promocion_aux != nullptr) {
					SendMessage(GetDlgItem(hPrinGlob, 1147), LB_ADDSTRING, 0, (LPARAM)Promocion_aux->Promocion_nombre);
					Promocion_aux = Promocion_aux->Promocion_sig;
				}
			}
		...
		if (LOWORD(wParam) == 1150) {//MODIFICAR LOS DATOS
			if (strcmp(NUsuario->usuarios_tipos, "Administrador") == 0)
			{
				EnableWindow(GetDlgItem(hwnd, 1144), true);
				EnableWindow(GetDlgItem(hwnd, 1151), true);
			}
		}

		if (LOWORD(wParam) == 1143) {
			EndDialog(hwnd, 0);
			DialogBox(hInstGlobal, MAKEINTRESOURCE(IDD_DIALOG3), hwnd, VentanaInicio);
		}
